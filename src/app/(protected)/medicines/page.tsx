...
import { MedicineCatalogView } from "@/features/medicines/components/medicine-catalog-view";
import {
...
export default async function MedicinesPage() {
  ...
  const mappedMedicines: MedicineCatalogItem[] = (dbMedicines ?? [])
    .filter(Boolean)
    .map((m) => ({
      id: m.id,
      name: m.name,
  ...
  const fallbackDefault = DEFAULT_CATALOG_MEDICINES?.[0];
  const initialMedicines = hasParacetamol
    ? mappedMedicines
    : fallbackDefault
      ? [fallbackDefault, ...mappedMedicines]
      : mappedMedicines;
