...
function TableHeader({ className, ...props }: React.ComponentProps<"thead">) {
  return <thead data-slot="table-header" className={cn("[&_tr]:border-b bg-muted/40 text-muted-foreground", className)} {...props} />;
}
...
function TableRow({ className, ...props }: React.ComponentProps<"tr">) {
  return (
    <tr
      data-slot="table-row"
      className={cn(
        "hover:bg-muted/40 has-aria-expanded:bg-muted/40 data-[state=selected]:bg-muted/50 border-b border-border/50 transition-colors",
        className,
      )}
      {...props}
    />
