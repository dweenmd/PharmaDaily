-- ---------------------------------------------------------------------------
-- Phase 6 / 4 — controlled drugs need someone qualified to dispense them
--
-- Until now, every warning in the POS was advisory — deliberately, because a
-- cashier can see the prescription in the customer's hand and the system
-- cannot, and a block staff cannot satisfy is a block staff learn to work
-- around.
--
-- Controlled drugs are the exception. Who may hand them over is not a judgement
-- about this customer; it is a fact about the person behind the counter, and
-- the system does know that. So this one is enforced.
--
-- SCOPE, stated plainly: this checks that the person COMPLETING the sale is
-- qualified to dispense. It is not the fuller flow where a cashier rings the
-- sale up and calls a pharmacist over to approve it at the till — that needs a
-- second person to authenticate mid-transaction, which is a real feature and
-- not this one. What it prevents is a cashier dispensing a controlled drug
-- alone, which is the thing that matters most.
--
-- Prescription-required (but not controlled) medicines stay advisory. A
-- prescription is a piece of paper the system cannot see, so demanding the
-- system approve it would only teach staff to click past it.
-- ---------------------------------------------------------------------------

create or replace function public.can_dispense_controlled()
returns boolean
language sql
security definer
stable
...
as $$
  select coalesce(
    public.current_user_role() in ('super_admin', 'branch_manager', 'pharmacist'),
    false
  );
$$;

revoke execute on function public.can_dispense_controlled() from public, anon;
grant execute on function public.can_dispense_controlled() to authenticated;

comment on function public.can_dispense_controlled() is
  'Roles qualified to hand over a controlled drug. A cashier alone is not.';
